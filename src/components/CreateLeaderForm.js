...
const CreateLeaderForm = () => {
  ...
  return (
    <Box sx={{ maxWidth: 400, mx: "auto", mt: 5 }}>
      ...
      <List>
        {lideres.map((lider) => (
          <ListItem key={lider.lider_identificacion}>
            <ListItemText
              primary={`${lider.lider_nombre} ${lider.lider_apellido} (ID: ${lider.lider_identificacion})`}
              secondary={`Recomendado: ${lider.recomendado_nombre || "N/A"} ${lider.recomendado_apellido || ""}`}
            />
          </ListItem>
        ))}
      </List>
